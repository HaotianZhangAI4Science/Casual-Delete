...
def train(model, batch_size, verbose=1, num_epoches=300):
    train_start = time()
    train_losses = []
    val_losses = []
    start_epoch = 0
    ...
    if config.train.resume_train:
        model = load(config, model, optimizer, scheduler)
        logger.info('load pretrained model from '.format(config.train.ckpt_path))
    ...
    for epoch in range(num_epoches):
        ...
        batch_cnt = 0
        all_loss = 0
        for mol_batch in train_loader:
            batch_cnt+=1
            mol_batch = mol_batch.to(args.device)
            loss, loss_frontier, loss_pos, loss_cls, loss_edge, loss_real, loss_fake, loss_surf = get_model_loss(model, mol_batch, config )
            all_loss += loss
            ...
            if batch_cnt % batch_size == 0:
                ...
                all_loss.backward()
                optimizer.step()
                batch_losses.append(all_loss.item())
                all_loss = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='./configs/causual_inference.yml')
    ...
    dataset = SurfLigandPairDataset(config.dataset.path, transform=transform, processed_path = config.dataset.name, name2id_path=config.dataset.name2id_path)
    ...
    val_loader = DataLoader(val_set, 1, shuffle=False, follow_batch=follow_batch, exclude_keys = collate_exclude_keys,)
    ...
    train(model, batch_size=config.train.batch_size)